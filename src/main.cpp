...
/*
  ...
  Modified to work on GD32 by Maximilian Gerhardt <maximilian.gerhardt@rub.de>
*/
...
void setup() {
  ...
  //gets a little bit of a speedup
...
uintDiff ducos1a(String lastblockhash, String newblockhash,
  ...
  SHA1_CTX baseCtx;
  SHA1Init(&baseCtx);
  SHA1Update(&baseCtx, (uint8_t *)lastblockhash.c_str(), lastblockhash.length());

  uint8_t buf[20];

  for (uintDiff ducos1res = 0; ducos1res < difficulty * 100 + 1; ducos1res++) {
    SHA1_CTX ctx = SHA1Copy(baseCtx);
    itoa(ducos1res, (char *)buf, 10);
    SHA1Update(&ctx, buf, strlen((char *)buf));
    SHA1Final(buf, &ctx);
    if (memcmp(buf, job, SHA1_HASH_LEN * sizeof(char)) == 0) {
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    Serial.print(String(ducos1result, 2) //print difficulty at which it works in binary
                 + "," 
                 + String(elapsedTime, 2)  //print elapsed time in binary
                 + "," 
                 + String(DUCOID) //print duco ID
                 + "\n");
