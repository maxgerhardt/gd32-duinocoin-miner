#!/usr/bin/env python3
"""
Duino-Coin Official GD32 Miner 2.75 © MIT licensed
modified from official AVR Miner 2.75
https://duinocoin.com
...
class Settings:
    ...
    SOC_TIMEOUT = 15
    REPORT_TIME = 120
    GD32_TIMEOUT = 4  # diff 6 * 100 / 196 h/s = 3.06
    BAUDRATE = 115200
    DATA_DIR = "Duino-Coin GD32 Miner " + str(VER)
    SEPARATOR = ","
    ENCODING = "utf-8"
    BLOCK = " ‖ "
    PICK = ""
...
try:
    if not Path(Settings.DATA_DIR + '/Settings.cfg').is_file():
        ...
        try:
            ...
            lang = config["GD32 Miner"]['language']
        except Exception:
            lang = 'english'
except:
    lang = 'english'
...
def get_prefix(symbol: str,
    ...
    H/s, 1000 => 1 kH/s
    """
    # always make sure it's a float
    val = float(val)
    #print("in get_prefix, symbol = " + str(symbol) + " val = " + str(val) + " accuracy = " + str(accuracy))
...
def load_config():
    ...
    if not Path(str(Settings.DATA_DIR) + '/Settings.cfg').is_file():
        ...
        while True:
            current_port = input(
                Style.RESET_ALL + Fore.YELLOW
                + get_string('ask_gd32port')
                + Fore.RESET + Style.BRIGHT)

            if current_port in port_names:
                gd32port += current_port
                confirmation = input(
                    Style.RESET_ALL + Fore.YELLOW
        ...
        gd32port = gd32port.split(',')
        print(Style.RESET_ALL + get_string('config_saved'))
        hashrate_list = [0] * len(gd32port)

    else:
        config.read(str(Settings.DATA_DIR) + '/Settings.cfg')
        username = config["GD32 Miner"]['username']
        gd32port = config["GD32 Miner"]['gd32port']
        gd32port = gd32port.replace(" ", "").split(',')
        donation_level = int(config["GD32 Miner"]['donate'])
        debug = config["GD32 Miner"]['debug']
        rig_identifier = config["GD32 Miner"]['identifier']
        Settings.SOC_TIMEOUT = int(config["GD32 Miner"]["soc_timeout"])
        Settings.GD32_TIMEOUT = float(config["GD32 Miner"]["gd32_timeout"])
        discord_presence = config["GD32 Miner"]["discord_presence"]
        shuffle_ports = config["GD32 Miner"]["shuffle_ports"]
        Settings.REPORT_TIME = int(config["GD32 Miner"]["periodic_report"])
        hashrate_list = [0] * len(gd32port)
...
def greeting():
    ...
    print(
        Style.DIM + Fore.MAGENTA
        + Settings.BLOCK + Style.NORMAL
        + Fore.RESET + get_string('gd32_on_port')
    ...
    while True:
        try:
            ...
            RPC.update(details="Hashrate: " + str(total_hashrate),
                       ...
                       + " accepted shares",
                       large_image="gd32miner",
                       large_text="Duino-Coin, "
                       + "a coin that can be mined with almost everything"
                       + ", including GD32 boards",
...
def share_print(id, type, accept, reject, total_hashrate,
    ...
    HH:MM:S |gd32N| ⛏ Accepted 0/0 (100%) ∙ 0.0s ∙ 0 kH/s ⚙ diff 0 k ∙ ping 0ms
    """
    if str(diff).isnumeric():
        try:
...
def mine_gd32(com, threadid, fastest_pool):
    global hashrate
    start_time = time()
    report_shares = 0
    last_report_share = 0
        ...
        while True:
            ...
            try:
                ser = Serial(com, baudrate=int(Settings.BAUDRATE),
                             timeout=float(Settings.GD32_TIMEOUT))
                """
        ...
        while True:
            try:
                ...
                Client.send(s, 'JOB'
                            ...
                            + str(username)
                            + Settings.SEPARATOR
                            + 'ESP32') # possible difficulty values: AVR, LOW, MEDIUM, NET, ESP8266, ESP8266H, ESP32, ARM, DUE, 
                            # see https://raw.githubusercontent.com/Bilaboz/duino-coin-pools/main/config/poolRewards.json
                            # we are closest to ESP32 by hash rate. automatic balancing will kick in anyways.
            ...
            while True:
                ...
                try:
                    ...
                    if result[0] and result[1]:
                        ...
                        raise Exception("No data received from GD32")
                except Exception as e:
            ...
            try:
                ...
                pretty_print('sys' + port_num(com),
                             get_string('mining_gd32_connection_error')
                             + Style.NORMAL + Fore.RESET
                             + ' (no response from the board: '
                             + f'{e}, please check the connection, '
                             + 'port setting or reset the GD32)', 'warning')
                break
            ...
            try:
                Client.send(s, str(num_res)
                            ...
                            + str(hashrate_t)
                            + Settings.SEPARATOR
                            + f'Official ESP32 Miner {Settings.VER}' # change to something else here
                            + Settings.SEPARATOR
                            + str(rig_identifier)
                            + Settings.SEPARATOR
                            + str(result[2]))
            ...
            title(get_string('duco_gd32_miner') + str(Settings.VER)
...
if __name__ == '__main__':
    init(autoreset=True)
    title(f"{get_string('duco_gd32_miner')}{str(Settings.VER)})")
    ...
    try:
        fastest_pool = Client.fetch_pool()
        threadid = 0
        for port in gd32port:
            Thread(target=mine_gd32,
                   args=(port, threadid,
                         fastest_pool)).start()
            threadid += 1
    except Exception as e:
        debug_output(f'Error launching GD32 thread(s): {e}')
